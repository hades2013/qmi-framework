  ...
  while(1)
  {
    ...
    if(pbits[0].revents)
    {
      ...
      if(ch == 'd')
      {
        ...
        pthread_attr_destroy(&xp->reader_tattr);
        free(xp);
        break;
      }
    }
 ...
 )
{
  ...
  if (pthread_attr_init(&xp->reader_tattr))
  {
    QMI_FW_LOGE("%s: Pthread reader thread attribute init failed\n", __func__);
    goto xport_open_close_pipe;
  }
  if (pthread_attr_setdetachstate(&xp->reader_tattr, PTHREAD_CREATE_DETACHED))
  {
    QMI_FW_LOGE("%s: Pthread Set Detach State failed\n", __func__);
    goto xport_open_close_pipe;
  }
  /* create reader thread */
  if(pthread_create(&xp->reader_tid, &xp->reader_tattr, reader_thread, (void *)xp))
  {
